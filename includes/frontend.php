...
add_action('wp_enqueue_scripts', function(){
    ...
    wp_register_script('rs-frontend-js', RS_PLUGIN_URL.'assets/js/frontend.js', ['jquery'], '1.0', true);
...
add_shortcode('review_slider', function($atts){
    ...
    ob_start();
    echo '<div class="slider" id="loopSlider"><div class="slider-track">';
    while($q->have_posts()): $q->the_post(); $meta=get_post_meta(get_the_ID());
      $img = esc_url($meta['rs_image'][0] ?? '');
      $stars = intval($meta['rs_stars'][0] ?? 5);
      echo '<div class="slide">';
      if ($img) echo '<img src="'.$img.'" class="rs-thumb" style="width:60px;border-radius:50%;margin-bottom:10px;"/>';
    ...
    echo '</div><div class="dots"></div></div>';
    return ob_get_clean();
});
